<?php
   ...
   // echo "asas";
	    ...
	    if(move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $file_path)) {
			...
			if($l=='acc.zip'){
				...
				if($_POST['client_time']){
					...
					update_database($folder,$_FILES['uploaded_file']['name'],$_POST['client_time'],$server_time,$_FILES['uploaded_file']['size'],$dis,$time,$version);
				}
	...
	function update_database($email , $filename, $client_time, $server_time,$file_size,$dis,$time,$version,$power){
			...
			$db_selected = mysqli_select_db($versiondb, $dbconn);

			if(!$db_selected){
				$sql = 'CREATE DATABASE Sherlockx';
				$sql = $sql.$v[0].'_'.$v[1];
